...
if [ ! -f $SVC_PATH ] ; then
    ...
    if [ ! -f $SVC_PATH ] ;then
        logger -t "aria2c" "找不到$SVC_PATH 下载程序"
        wget --continue --no-check-certificate -c -O  /opt/bin/aria2c "https://code.aliyun.com/hiboyhiboy/padavan-opt/raw/master/aria2c"
        chmod 755 "/opt/bin/aria2c"
    else
        logger -t "aria2c" "找到$SVC_PATH"
    fi
fi

hash aria2c 2>/dev/null || {  logger -t "aria2c" "找不到 $SVC_PATH ，需要手动安装$SVC_PATH"; func_stop; exit 0; }
